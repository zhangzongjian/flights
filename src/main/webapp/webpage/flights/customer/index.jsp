...
$(document).ready(function(){
  ...
  $(".add").click(function(){
	  location.href = "${contextPath}/customer/intoAdd";
  });
  ...
  $(".update").click(function(){
	  ...
	  location.href = "${contextPath}/customer/intoUpdate?id="+id+"&page=${pager.page }";
  });

  $(".delete").click(function(){
	  if(confirm("确定删除多条记录？")) {
		  if($("[name='id']:checked").length == 0) return;
		  var url = "${contextPath}/customer/delete?";
...
<body>
    ...
    <table class="tablelist">
        ...
        <tbody>
        <c:forEach items="${pager.datas }" var="customer">
	        <tr>
	        ...
	        <td>
		        ...
		        <a href="${contextPath}/customer/delete?id=${customer.id }&page=${pager.page }" onclick="javascript:if(!confirm('确定删除该记录？')) return false;" class="tablelink">删除</a>
	        </td>
	        </tr> 
        </c:forEach>
        </tbody>
	...
	function goPage(pageNum) {
		...
		if(pageNum > 0 && pageNum <= '${pager.pageSum }') {
			location.href = "${contextPath}/customer/index?limit=${pager.limit}&page="+pageNum;
		}
	}
	//每页显示条目
	function setLimit(limit) {
		if(limit > 0 && limit <= 1000) {
			location.href = "${contextPath}/customer/index?limit="+limit;
		}
	}

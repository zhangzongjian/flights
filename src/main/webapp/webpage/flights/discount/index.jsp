...
$(document).ready(function(){
  ...
  $(".add").click(function(){
	  location.href = "${contextPath}/discount/intoAdd";
  });
  ...
  $(".update").click(function(){
	  ...
	  location.href = "${contextPath}/discount/intoUpdate?id="+id+"&page=${pager.page }";
  });

  $(".delete").click(function(){
	  if(confirm("确定删除多条记录？")) {
		  if($("[name='id']:checked").length == 0) return;
		  var url = "${contextPath}/discount/delete?";
...
<body>
    ...
    <table class="tablelist">
        ...
        <c:forEach items="${pager.datas }" var="discount">
	        ...
	        <td>
		        <a href="${contextPath}/discount/detail?id=${discount.id }&page=${pager.page }" class="tablelink">查看</a>     
		        <a href="${contextPath}/discount/intoUpdate?id=${discount.id }&page=${pager.page }" class="tablelink">修改</a>
		        <a href="${contextPath}/discount/delete?id=${discount.id }&page=${pager.page }" onclick="javascript:if(!confirm('确定删除该记录？')) return false;" class="tablelink">删除</a>
	        </td>
	        </tr> 
        </c:forEach>
        </tbody>
	...
	function setLimit(limit) {
		if(limit > 0 && limit <= 1000) {
			location.href = "${contextPath}/discount/index?limit="+limit;
		}
	}

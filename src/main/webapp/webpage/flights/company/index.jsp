...
$(document).ready(function(){
  ...
  $(".add").click(function(){
	  location.href = "${contextPath}/company/intoAdd";
  });
  ...
  $(".update").click(function(){
	  ...
	  location.href = "${contextPath}/company/intoUpdate?id="+id+"&page=${pager.page }";
  });

  $(".delete").click(function(){
	  if(confirm("确定删除多条记录？")) {
		  if($("[name='id']:checked").length == 0) return;
		  var url = "${contextPath}/company/delete?";
...
<body>
	...
	function goPage(pageNum) {
		...
		if(pageNum > 0 && pageNum <= '${pager.pageSum }') {
			location.href = "${contextPath}/company/index?limit=${pager.limit}&page="+pageNum;
		}
	}
	//每页显示条目
	function setLimit(limit) {
		if(limit > 0 && limit <= 1000) {
			location.href = "${contextPath}/company/index?limit="+limit;
		}
	}

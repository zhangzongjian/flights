...
$(document).ready(function(){
  ...
  $(".add").click(function(){
	  location.href = "${contextPath}/order/intoAdd";
  });
  ...
  $(".update").click(function(){
	  ...
	  location.href = "${contextPath}/order/intoUpdate?id="+id+"&page=${pager.page }";
  });

  $(".delete").click(function(){
	  if(confirm("确定删除多条记录？")) {
		  if($("[name='id']:checked").length == 0) return;
		  var url = "${contextPath}/order/delete?";
...
<body>
    ...
    <table class="tablelist">
        ...
        <tbody>
        <c:forEach items="${pager.datas }" var="order">
	        <tr>
	        ...
	        <td>
		        ...
		        <a href="${contextPath}/order/delete?id=${order.id }&page=${pager.page }" onclick="javascript:if(!confirm('确定删除该记录？')) return false;" class="tablelink">删除</a>
	        </td>
	        </tr> 
        </c:forEach>
        </tbody>
	...
	function setLimit(limit) {
		if(limit > 0 && limit <= 1000) {
			location.href = "${contextPath}/order/index?limit="+limit;
		}
	}
